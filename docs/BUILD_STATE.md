...
## Welcome Case milestone (v7)
- Replaced the first-run five-skill placement screen with Northstar Coffee: The Downtown Location, the executable Chapter 0 guided mini-case.
- The ten-stage flow persists after every interaction, reconciles deterministic economics, provides one-retry arithmetic hints, teaches exhibit observation versus implication, and transitions directly to Chapter 1.
- Welcome responses are conservative guided acquisition calibration only and cannot create mastery, transfer, retention, integration, or readiness evidence. Existing meaningful learner histories satisfy the walkthrough migration without being forced backward.
