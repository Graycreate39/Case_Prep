...
export const caseActionSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("submit-insight"), insightId: z.string().optional(), text: z.string().trim().min(20).max(4_000) }),
  z.object({ type: z.literal("submit-calculation"), calculationId: z.string(), answer: z.number().finite(), answerText: z.string().max(80).optional(), equation: z.string().max(500), unit: z.string().max(80) }),
...
export type PublicCaseView = {
  ...
  pendingInsightCount:number; calculations:{id:string;prompt:string}[];
};
...
export function projectPublicCase(session:FrozenCaseSession):PublicCaseView {
  ...
  return {sessionId:session.id,title:spec.title,objective:spec.objective,stageId:stage.id,stageObjective:stage.objective,mode:session.mode,poolRole:spec.poolRole,
    ...
    turns:session.turns,hintsUsed:session.hintsUsed,complete:Boolean(session.completedAt),pendingInsightCount:stage.requiredInsights.filter(id=>!session.completedInsightIds.includes(id)).length,calculations:stage.id==="analysis"?spec.calculations.map(({id,prompt})=>({id,prompt})):[]};
}
...
export function applyCaseAction(current:FrozenCaseSession,input:unknown):FrozenCaseSession {
  ...
  if(action.type==="submit-insight"){
    const pending=stage.requiredInsights.filter(id=>!session.completedInsightIds.includes(id));
    const matched=pending.find(id=>insightHasMinimumEvidence(id,action.text));
    if(!matched)throw new Error("Insight needs decision-relevant reasoning and supporting evidence");
    session.completedInsightIds=[...new Set([...session.completedInsightIds,matched])]; session.turns.push(turn("candidate",action.text,"insight"));
  ...
  if(action.type==="submit-calculation"){
    ...
    const numeric=gradeNumber(action.answerText??action.answer,calculation.answer,calculation.tolerance), unitsMatch=action.unit.trim().toLowerCase()===calculation.unit.toLowerCase();
  ...
  if(action.type==="advance"){
    ...
    if(missing.length)session.turns.push(turn("interviewer","Before moving on, connect your analysis to the client decision and state the next step."));
