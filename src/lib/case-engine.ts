...
export const caseModeSchema = z.enum(["practice", "strict", "pressure"]);
...
export const caseActionSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("submit-insight"), insightId: z.string(), text: z.string().trim().min(1).max(4_000) }),
...
export type FrozenCaseSession = {
  ...
  revealedFactIds: string[]; availableExhibitIds: string[]; completedInsightIds: string[]; calculationResults: CalculationResult[];
  hintsUsed: number; startedAt: string; completedAt?: string; turns: CaseTurn[];
};

export type PublicCaseView = {
  sessionId: string; title: string; objective: string; stageId: string; stageObjective: string; mode: CaseMode;
...
export function createCaseSession(specInput:unknown, mode:CaseMode="practice"):FrozenCaseSession {
  ...
  return {id:crypto.randomUUID(),caseId:spec.id,caseVersion:spec.version,specSnapshot:spec,mode,stageId:stage.id,
    ...
    completedInsightIds:[],calculationResults:[],hintsUsed:0,startedAt:new Date().toISOString(),
...
export function projectPublicCase(session:FrozenCaseSession):PublicCaseView {
  ...
  return {sessionId:session.id,title:spec.title,objective:spec.objective,stageId:stage.id,stageObjective:stage.objective,mode:session.mode,
    facts:spec.clientFacts.filter(f=>factIds.has(f.id)).map(({id,text})=>({id,text})),exhibits:spec.exhibits.filter(e=>exhibitIds.has(e.id)),
...
function interviewerReply(session:FrozenCaseSession,text:string){
  ...
  const available=spec.clientFacts.find(f=>session.revealedFactIds.includes(f.id)&&words.some(w=>f.text.toLowerCase().includes(w)));
  if(available)return available.text;
  if(spec.hiddenFacts.some(f=>words.some(w=>f.text.toLowerCase().includes(w))))return "I don’t have additional information to share on that yet.";
...
export function applyCaseAction(current:FrozenCaseSession,input:unknown):FrozenCaseSession {
  ...
  if(action.type==="request-hint"){
    session.hintsUsed++; session.turns.push(turn("system",session.mode==="strict"?"Hint recorded. Strict mode does not provide coaching during the case.":`Focus on the stage objective: ${stage.objective}`));
  }
...
export function evaluateCompletedCase(session:FrozenCaseSession){
  ...
  return {caseId:session.caseId,caseVersion:session.caseVersion,mode:session.mode,calculationAccuracy:accuracy,insightCoverage,hintsUsed:session.hintsUsed,
