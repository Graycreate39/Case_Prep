...
export function DiagnosticAssessment({onAttempt,onComplete}:{onAttempt:(attempt:Attempt)=>void;onComplete:()=>void}){
 ...
 if(index!==-1){const task=tasks[index];return <SkillLab key={task.id} exerciseId={task.id} onAttempt={onAttempt} onFinished={correct=>{const next=[...results,correct];setResults(next);if(index===tasks.length-1)onComplete();else setIndex(index+1)}}/>}
